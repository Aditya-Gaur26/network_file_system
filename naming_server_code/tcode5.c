...
void add_server_to_subtree(TreeNode* node, int server_id, int for_backup){
    if (!node) return;

    add_server_to_node(node, server_id, for_backup, -1);

    if (node->is_directory && node->children) {
        for (int i = 0; i < node->child_count; i++) {
            add_server_to_subtree(node->children[i], server_id, for_backup);
        }
    }
}
...
void add_remaining_path_to_tree(TreeNode* parent, const char* remaining_path) {
                        ...
                        if(strcmp(type1, "naming_server_related") == 0) {
                            ...
                            if(strcmp(request_status, "success") == 0) {
                                ...
                                if(strcmp(req_operation, "create_empty") == 0){
                                    ...
                                    fprintf(stderr, "%s\n", req_path);
                                    TreeNode* node = find_node(nm->root, req_path);
                                    ...
                                    if(node == NULL){
                                        ...
                                        if(insertion_point){
                                           add_remaining_path_to_tree(insertion_point, remaining_path);
                                        }else{
                                        ...
                                        add_server_to_node(node, sid, 0, -1);
                                    }
                                }
                            ...
                            if(cl->pending_requests[i].reqid != -1 && cl->pending_requests[i].reqid == client_request_id) {
                                ...
                                for(int i = 0; i < nm->storage_servers[sidx]->current_load; i ++) {
                                    if(nm->storage_servers[sidx]->processing_requests[i].reqid != -1 && nm->storage_servers[sidx]->processing_requests[i].reqid == client_request_id) {
                                        for(int j = 0; j < nm->storage_servers[sidx]->current_load; j ++) {
                                            nm->storage_servers[sidx]->processing_requests[j - 1] = nm->storage_servers[sidx]->processing_requests[j - 1];
                                        }
...
void* handle_backup(void* arg){
    ...
    for(int i = 0; i < ss->path_count; i++){
        ...
        if(node->is_written_to){
            ...
            strcpy(paths[cnt++], ss->pending_backup_paths[i]);
        }
        else{
    ...
    for(int i = 0; i < cnt; i++){
        ...
        strcpy(ss->pending_backup_paths[i], paths[i]);
    }
...
int handle_storage_server_registration(int client_socket, json_object* request) {
    ...
    StorageServer* ss;
    int exist_id;
    if(json_object_object_get_ex(request, "ssid", &existing_id_obj) && (exist_id = json_object_get_int(existing_id_obj)) != -1) {
        ...
        if(ss->backup1 == NULL && ss->backup2 == NULL){
            printf("Thers is no backup for this Storage sever");
        }
        else{
            StorageServer* backup = (ss->backup1 != NULL) ? (ss->backup1) : (ss->backup2);

            const char* path1 = "myserver/";
...
int handle_client_request(Client *cl, int client_socket, json_object *request) {
    ...
    if (strcmp(operation, "create") == 0) {
        ...
        json_object_object_add(request, "request_id", json_object_new_int(-1));
        json_object_object_add(request, "request_code", json_object_new_string("delete"));
        json_object_object_add(request, "path", json_object_new_string(path2));

        StorageServer* ss = nm->storage_servers[enode_server];

        if(ss->backup1) send_message(ss->backup1->sock, request);
        if(ss->backup2) send_message(ss->backup2->sock, request);
        json_object_put(request);
